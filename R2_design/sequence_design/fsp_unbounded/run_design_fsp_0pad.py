...
def generate_sequences(model_type,design_seq_len=145,target_function='h2k',suffix='',use_maxmin=False):
    ...
    if target_function == 'max1':
        suffix = 'max1'
    elif target_function == 'min1':
        suffix = 'min1'
    else:
        suffix = ''
    ...
    if design_seq_len < 145:
        if len(suffix) > 0:
            design_dir = f'designed_seqs/{DESIGN_TYPE}_minimal/{suffix}/{model_type}/minimal_{design_seq_len}'
        else:
            design_dir = f'designed_seqs/{DESIGN_TYPE}_minimal/{model_type}/minimal_{design_seq_len}'
        if not os.path.exists(design_dir):
            os.makedirs(design_dir)
    else:
        if len(suffix) > 0:
            design_dir = f'designed_seqs/{DESIGN_TYPE}/{suffix}/{model_type}'
        else:
    ...
    for cell_type_idx, cell_type in enumerate(CELL_TYPES):
        ...
        output_filepath = os.path.join(design_dir, f"designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_seqs.fasta")
        ...
        # Define target loss - could add K.clip(x,min_val,max_val)
        # this version maximizes only target output, ignores other output
        def target_loss_func_max1(model_preds):
            # model_preds has dimensions (n_seqs, n_outputs)
            target_score = - tensorflow.reduce_mean(model_preds[:, cell_type_idx])
            return target_score
        
        def target_loss_func_min1(model_preds):
            # model_preds has dimensions (n_seqs, n_outputs)
            target_score = tensorflow.reduce_mean(model_preds[:, cell_type_idx])
            return target_score
        ...
        elif target_function == 'max1':
            target_loss_func = target_loss_func_max1
        elif target_function == 'min1':
            target_loss_func = target_loss_func_min1
        else:
            print('Error: target function not recognized.')
            sys.exit(1)
        ...
        sort_inds = np.argsort(pred_vals[:,cell_type_idx] - pred_vals[:,1-cell_type_idx])[::-1]
        ...
        fig.savefig(
            os.path.join(design_dir, f"designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_{model_type}_train_history.png"),
        ...
        fig.savefig(
            os.path.join(design_dir, f"designed__{model_type}_{DESIGN_TYPE}cell_type_{cell_type_idx}_{cell_type}_{model_type}_bar_signal.png"),
            dpi=200, bbox_inches='tight',
        )
        plt.close(fig)
        ...
        fig.savefig(
            os.path.join(design_dir, f"designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_seq_bitmap.png"),
            dpi=200, bbox_inches='tight',
        )
        plt.close(fig)
        ...
        fig.savefig(
            os.path.join(design_dir, f"designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_seqs.png"),
            dpi=200, bbox_inches='tight',
        )
        plt.close(fig)

        # Save training history data
        train_hist_filepath = os.path.join(design_dir, f"designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_train_hist.pickle")
        ...
        for seq_idx, seq in enumerate(seqs):
            seq_id = f'designed_{model_type}_{DESIGN_TYPE}_target_{cell_type_idx}_{cell_type}_seq_{seq_idx}'
...
if __name__ == '__main__':
    ...
    if not pretrained:
        generate_sequences(model_type,design_seq_len=design_seq_len,target_function=target_function,suffix=suffix,use_maxmin=use_maxmin)
    ...
    seq_df = pd.DataFrame(columns=['sequence_name','sequence','cell_type'])
    ...
    for cell_type_idx, cell_type in enumerate(CELL_TYPES):
        ...
        seq_file = f'{design_dir}/designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_seqs.fasta'
            ...
            for line in f:
                if line[0] == '>':
                    ...
                    if line_cnt == n_lines_per_seq:
                        seq_df = pd.concat([seq_df, pd.DataFrame({'sequence_name':seq_name,'sequence':seq,'cell_type':cell_type}, index=[0])], ignore_index=True)
