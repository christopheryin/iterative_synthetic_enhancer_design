...
def generate_sequences_target(model_type,design_seq_len=145,target_function='h2k',suffix='',use_maxmin=False):
    ...
    max_pred_k2h = -MODEL_TYPE_DF.loc[model_type]['max_pred_k2h'] # this is already negative, so want to flip sign
    N_TARGET_VALS = 4
    MIN_VAL = 2
    hepg2_target_vec = np.linspace(MIN_VAL,max_pred_h2k,N_TARGET_VALS)
    k562_target_vec = np.linspace(MIN_VAL,max_pred_k2h,N_TARGET_VALS)
    target_vecs = [hepg2_target_vec,k562_target_vec]
    ...
    if design_seq_len < 145:
        ...
        if len(suffix) > 0:
            design_dir = f'designed_seqs/{DESIGN_TYPE}_tgt/{suffix}/{model_type}'
        else:
            design_dir = f'designed_seqs/{DESIGN_TYPE}_tgt/{model_type}'
        if not os.path.exists(design_dir):
            os.makedirs(design_dir)
    ...
    for cell_type_idx, cell_type in enumerate(CELL_TYPES):
        ...
        target_vec = target_vecs[cell_type_idx]

        output_filepath = os.path.join(design_dir, f"designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_tgt_seqs.fasta")
        ...
        for target in target_vec:
            ...
            target_loss_func = get_target_loss_func(target)
            ...
            fig.savefig(
                os.path.join(design_dir, f"designed__{model_type}_{DESIGN_TYPE}cell_type_{cell_type_idx}_{cell_type}_{model_type}_tgt{target:.2f}_bar_signal.png"),
                dpi=200, bbox_inches='tight',
            )
            plt.close(fig)
            ...
            for seq_idx, seq in enumerate(seqs):
                seq_id = f'designed_{model_type}_{DESIGN_TYPE}_target_{cell_type_idx}_{cell_type}_tgt_{(target*h2k_dir):.2f}_seq_{seq_idx}'
...
if __name__ == '__main__':
    ...
    hepg2_target_vec = np.linspace(0,max_pred_h2k,5)
    k562_target_vec = np.linspace(0,max_pred_k2h,5)
    target_vecs = [hepg2_target_vec,k562_target_vec]
    ...
    for cell_type_idx, cell_type in enumerate(CELL_TYPES):
        ...
        seq_file = f'{design_dir}/designed_{model_type}_{DESIGN_TYPE}_cell_type_{cell_type_idx}_{cell_type}_tgt_seqs.fasta'
    ...
    # scatterplot of log2(H2K)_pred vs target
    fig,ax = plt.subplots(figsize=(5,3))
    # plot scatterplot colored by cell type
    sns.scatterplot(data=seq_df, x='target', y='log2(H2K)_pred', hue='cell_type', palette=PALETTE, ax=ax)
    ax.set_xlabel('target. log2(HEPG2/K562)')
    ax.set_ylabel('pred. log2(HEPG2/K562)')
    ax.set_title(f'{model_type} {DESIGN_TYPE} designed seq preds vs target')
    # hide legend
    ax.legend().remove()
    # add grid lines
    plt.grid(axis='y',zorder=-1)
    ...
    fig.savefig(
        f"{design_dir}/designed_{model_type}_{DESIGN_TYPE}_pred_vs_tgt_scatter.png",
        # 'temp.png',
        dpi=120,
        bbox_inches='tight',
    )
